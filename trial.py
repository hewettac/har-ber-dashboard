...
if uploaded_file:
    ...
    def custom_yard_group(yardline):
        if pd.isna(yardline):
            return "Unknown"
        # Ensure yardline is numeric for comparison
        if not isinstance(yardline, (int, float)):
            return "Other"
        if 0 >= yardline >= -9:
            return "0 - -9"
        elif -10 >= yardline >= -19:
    ...
    with tab5:
        ...
        # Feature Engineering
        # -------------------------
        def add_features(df_to_process):
            df_to_process = df_to_process.copy()

            df_to_process["distance_bucket"] = pd.cut(
                df_to_process["distance"],
                bins=[0, 3, 7, 20],
                labels=[0, 1, 2]
            ).astype(float)

            df_to_process["field_zone"] = pd.cut(
                df_to_process["yardline"],
                bins=[-50, -20, 20, 50],
